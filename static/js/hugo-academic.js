...
(function($){
    ...
    $('#navbar-main li.nav-item a').on('click', function(event) {
        ...
        if ( hash && $(hash).length && ($("#homepage").length > 0)) {
            ...
            var pos = $(hash).offset().top - navbar_offset;
            if(hash == "#top") {
                pos = 0;
            }
    ...
    $(window).on('scroll', function() {
        if($(window).scrollTop() > navbar_offset) {
            $("#navbar-main").addClass("navbar-compact");
        } else {
            $("#navbar-main").removeClass("navbar-compact");
        }
    });
